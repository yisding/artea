...
def service(tmp_path, cfg):
    ...
    yield url, triggers, state, store
    httpd.shutdown()
    httpd.server_close()
...
def test_policy_404_before_first_sync(service):
    url, _, _, _ = service  # store empty, fallback file does not exist
