...
PATCH_FILE=""
trap 'rm -f "$RESP" "$CLIENT_SETUP_FILE" "$SETUP_SCRIPT_FILE" ${PATCH_FILE:+"$PATCH_FILE"}' EXIT
...
render_template() { # <template path> <output path> ; substitute the __X__ placeholders
...
render_template policy/CLIENT-SETUP.md.template "${CLIENT_SETUP_FILE}"
ensure_generated_file "${CLIENT_SETUP_FILE}" CLIENT-SETUP.md "Generated by Artea bootstrap"
# One-shot client setup script (detects tools, takes a PAT, merges config).
render_template policy/setup.sh.template "${SETUP_SCRIPT_FILE}"
ensure_generated_file "${SETUP_SCRIPT_FILE}" setup.sh "Generated by Artea bootstrap"
