...
| Behind a TLS-terminating LB (AWS ELB/ALB), the UI/login or package links redirect to `http://` (301/302 downgrade or redirect loop) | The LB ends HTTPS and forwards to the gateway over plain HTTP, so Gitea/devpi see `http`; the gateway honors the LB's `X-Forwarded-Proto` and forwards the real scheme | Confirm the LB sends `X-Forwarded-Proto: https` (ALB does by default). Verify the gateway relays it: `curl -s -H 'X-Forwarded-Proto: https' http://<gateway>/api/v1/user -o /dev/null -w '%{http_code}'` and check Gitea then emits `https://` `Location` headers. `global.baseUrl` must be the `https://` public URL |
