...
```
http://localhost:8080/api/packages/artea/pypi/
```
...
pip install artea-hello   # private, served by Gitea (shadows PyPI)
pip index versions artea-hello   # shows ONLY private versions for private names
```
...
`poetry add six` and `poetry add artea-hello` then both resolve through the
...
[artea]
repository = http://localhost:8080/api/packages/artea/pypi/
...
Requires a `write:package` token and write permission in the `artea` org.
Or without a `.pypirc`:
...
| `401`/`403` on `twine upload` | Token is `read:package` only, or user lacks write permission in the `artea` org |
