...
export interface PolicySourceConfig {
  /** Path to the policy file (the shared /policy volume). Mutually exclusive with policy_url. */
  policy_file?: string;
  /** policy-sync HTTP endpoint to poll (K8s: no shared volume). Mutually exclusive with policy_file. */
  policy_url?: string;
  /** Shared upstream package policy path. Mutually exclusive with upstream_policy_url. */
